...
function monthDiffInclusive(start, end) {
  if (!start || !end || compareMonthDates(start, end) > 0) return 0;
...
export function calculateDebtInterestSummary({ debt, payments = [], rateChanges = [], now = new Date() }) {
  ...
  const principalAdjustments = new Map();
  for (const payment of payments) {
    if (!['principal', 'clearance'].includes(payment.payment_type)) continue;
    const monthKey = formatMonthKey(payment.payment_date);
    if (!monthKey) continue;

    const current = principalAdjustments.get(monthKey) || { total: 0, cleared: false };
    current.total += Number(payment.amount || 0);
    current.cleared = current.cleared || payment.payment_type === 'clearance';
    principalAdjustments.set(monthKey, current);
  }

  const normalizedRateChanges = [...rateChanges]
    .map((change) => ({
      effectiveMonth: toMonthDate(change.effective_month),
  ...
  let principal = Number(debt.principal || debt.current_principal || 0);
  ...
  for (let month = firstInterestMonth; compareMonthDates(month, currentMonth) <= 0; month = addMonths(month, 1)) {
    ...
    const adjustment = principalAdjustments.get(monthKey);
    if (adjustment) {
      principal = adjustment.cleared ? 0 : Math.max(0, principal - adjustment.total);
    }
  ...
  const accruedMonths = interestMonths.filter((month) => month.is_accrued);
  ...
  const currentMonthEntry = interestMonths.find((month) => month.month === formatMonthDate(currentMonth));
...
export function getInterestRangeLabel(summary) {
  if (!summary.interest_start_month) return null;
  if (!summary.interest_to_month) return summary.interest_start_month;
  return `${summary.interest_start_month}:${summary.interest_to_month}`;
}
