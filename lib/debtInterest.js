 ...
 * Used to handle floating-point rounding in EMI amortization calculations.
 */
...
export function calculateEmiSchedule({ principal, interestRate, emiAmount, maxMonths = 360 }) {
  ...
  for (let i = 1; i <= maxMonths && balance > BALANCE_EPSILON; i++) {
    ...
    if (principalPortion <= 0) {
      // EMI does not cover interest; schedule won't close
      schedule.push({
        month_number: i,
        ...
        emi,
        closing_balance: balance - principalPortion,
      });
