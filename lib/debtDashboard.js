...
export async function ensureDebtMetadataColumns(sql) {
  ...
  await sql`ALTER TABLE debts ADD COLUMN IF NOT EXISTS emi_amount NUMERIC(14,2)`;
}
...
/**
 * Build a flat, date-sorted list of lifecycle events for the timeline graph.
 * - 'added'  → one entry per debt using its start_date
 * - 'cleared' → one entry per clearance payment row
 */
export function buildDebtTimeline(allDebts, allPayments) {
  const events = [];

  for (const debt of allDebts) {
    const date = toDateKey(debt.start_date);
    if (!date) continue;
    events.push({
      type: 'added',
      date,
  ...
  for (const payment of allPayments) {
    if (payment.payment_type !== 'clearance') continue;
    const date = toDateKey(payment.payment_date);
    if (!date) continue;
    events.push({
      type: 'cleared',
      date,
